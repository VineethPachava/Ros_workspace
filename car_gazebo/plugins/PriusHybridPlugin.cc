...
namespace gazebo
{
  class RoadstarHybridPluginPrivate
  {
    /// \enum DirectionType
    ...
    /// \brief Mutex to protect updates=========understand this**************
...
/////////////////////////////////////////////////
RoadstarPlugin::RoadstarPlugin()
    : dataPtr(new RoadstarHybridPluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "RoadstarHybridPlugin");
  ros::NodeHandle nh;
  this->dataPtr->controlSub = nh.subscribe("roadstar", 10, &RoadstarPlugin::OnPriusCommand, this);

  this->dataPtr->directionState = RoadstarHybridPluginPrivate::FORWARD;
...
void RoadstarPlugin::OnPriusCommand(const prius_msgs::Control::ConstPtr &msg)
{
  ...
  switch (msg->shift_gears)
  {
    case prius_msgs::Control::NEUTRAL:
      this->dataPtr->directionState = RoadstarHybridPluginPrivate::NEUTRAL;
      break;
    case prius_msgs::Control::FORWARD:
      this->dataPtr->directionState = RoadstarHybridPluginPrivate::FORWARD;
      break;
    case prius_msgs::Control::REVERSE:
      this->dataPtr->directionState = RoadstarHybridPluginPrivate::REVERSE;
      break;
    default:
      break;
  }
...
/////////////////////////////////////////////////
void RoadstarPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  gzwarn << "RoadstarPlugin loading params" << std::endl;
  // shortcut to this->dataPtr
  RoadstarHybridPluginPrivate *dPtr = this->dataPtr.get();
  ...
  this->dataPtr->node.Subscribe("/roadstar/reset",
      &RoadstarPlugin::OnReset, this);
  this->dataPtr->node.Subscribe("/roadstar/stop",
      &RoadstarPlugin::OnStop, this);

  this->dataPtr->node.Subscribe("/cmd_vel", &RoadstarPlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe("/cmd_gear",
      &RoadstarPlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe("/cmd_mode",
      &RoadstarPlugin::OnCmdMode, this);

  this->dataPtr->posePub = this->dataPtr->node.Advertise<ignition::msgs::Pose>(
      "/roadstar/pose");
  this->dataPtr->consolePub =
    this->dataPtr->node.Advertise<ignition::msgs::Double_V>("/console");
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&RoadstarPlugin::Update, this));
// figure out what the heck is keyboardpress
  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("~/keyboard/keypress",
        &RoadstarPlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub =
    this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/world_control");

  this->dataPtr->node.Subscribe("/keypress", &RoadstarPlugin::OnKeyPressIgn,
      this);
}

/////////////////////////////////////////////////
void RoadstarPlugin::OnCmdVel(const ignition::msgs::Pose &_msg)
{
...
/////////////////////////////////////////////////
void RoadstarPlugin::OnCmdGear(const ignition::msgs::Int32 &_msg)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  this->dataPtr->directionState =
      static_cast<RoadstarHybridPluginPrivate::DirectionType>(state);
}

/////////////////////////////////////////////////
void RoadstarPlugin::OnCmdMode(const ignition::msgs::Boolean &/*_msg*/)
{
  // toggle ev mode
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void RoadstarPlugin::KeyControlTypeA(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void RoadstarPlugin::KeyControlTypeB(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  switch (_key)
  {
    ...
    {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != RoadstarHybridPluginPrivate::REVERSE)
        this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent =
          std::min(this->dataPtr->gasPedalPercent, 1.0);
...
/////////////////////////////////////////////////
void RoadstarPlugin::KeyControl(const int _key)
{
...
/////////////////////////////////////////////////
void RoadstarPlugin::OnKeyPress(ConstAnyPtr &_msg)
{
  this->KeyControl(_msg->int_value());
}

/////////////////////////////////////////////////
void RoadstarPlugin::OnKeyPressIgn(const ignition::msgs::Any &_msg)
{
  this->KeyControl(_msg.int_value());
}

/////////////////////////////////////////////////
void RoadstarPlugin::OnReset(const ignition::msgs::Any & /*_msg*/)
{
...
/////////////////////////////////////////////////
void RoadstarPlugin::OnStop(const ignition::msgs::Any & /*_msg*/)
{
...
void RoadstarPlugin::Update()
{
  ...
  bool neutral = dPtr->directionState == RoadstarHybridPluginPrivate::NEUTRAL;
  ...
  if ((curTime - this->dataPtr->lastMsgTime) > .5)
  {
    ...
    if (this->dataPtr->directionState == RoadstarHybridPluginPrivate::FORWARD)
      consoleMsg.add_data(1.0);
    else if (this->dataPtr->directionState == RoadstarHybridPluginPrivate::REVERSE)
      consoleMsg.add_data(2.0);
    else if (this->dataPtr->directionState == RoadstarHybridPluginPrivate::NEUTRAL)
...
/////////////////////////////////////////////////
void RoadstarPlugin::UpdateHandWheelRatio()
{
...
double RoadstarPlugin::CollisionRadius(physics::CollisionPtr _coll)
{
...
/////////////////////////////////////////////////
double RoadstarPlugin::GasTorqueMultiplier()
{
...
GZ_REGISTER_MODEL_PLUGIN(RoadstarPlugin)
