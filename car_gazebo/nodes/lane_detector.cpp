...
cv::Mat edgeDetector(cv::Mat img_noise) {
  ...
  // Binarize gray image
  cv::threshold(output, output, 35, 255, cv::THRESH_BINARY);
...
// ===============================================================================================
// change parameters here for controlling region of intrest
cv::Mat mask(cv::Mat img_edges) {
  cv::Mat output;
  cv::Mat mask = cv::Mat::zeros(img_edges.size(), img_edges.type());
  cout << "Width : " << img_edges.cols << endl;
  cout << "Height: " << img_edges.rows << endl;
  cv::Point pts[4] = {
      cv::Point(350,300),
      cv::Point(400, 300),
      cv::Point(600, 500),
...
void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    try
    {
      ...
      std::cout<<"I am converting "<<"\n";
    }
    catch (cv_bridge::Exception& e)
    {
      ROS_ERROR("cv_bridge exception: %s", e.what());
      std::cout<<"It failed to convert "<<"\n";
      return;
    }
...
image_sub_ = it_.subscribe("/front_camera/image_raw", 1, imageCb) ;
std::cout<<"I am subscribing "<<"\n";
